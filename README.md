...
$x\left(t\right)=sin\left(2\pitf_r\right)+sin\left(2\pitf_c\right)$
...
####时域信号转换成频谱实现步骤：
（1）用fft函数对数字信号的y向量做快速傅里叶变换，再用fftshift对得到的结果进行调整，得到0频率分量在中央的函数图像fft_y。fftshift(abs(fft(y)))./(Len/2);
（2）计算fft_y对应的频率下标，Len代表取样的点数。
设N为取样点数，根据第n点对应频率f=n\times\frac{f_s}{N}。
